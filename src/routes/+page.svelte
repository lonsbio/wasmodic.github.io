  ...
  <!-- Reference Links Grid -->
    <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6 mb-8">
        {#each examples as link}
            <a href={link.link} target="_blank" rel="noopener noreferrer" class="card preset-tonal-primary hover:preset-filled-primary-950 transition-all duration-200 p-6 space-y-2 group">
                <h3 class="h4 group-hover:underline">{link.name}</h3>
                <p class="opacity-70">{link.description}</p>
            </a>
        {/each}
  ...
  <!-- Examples Links Grid -->
    ...
    <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6 mb-8">
        {#each references as link}
            <a href={link.link} target="_blank" rel="noopener noreferrer" class="card preset-tonal-secondary-50 hover:preset-filled-secondary-950 transition-all duration-200 p-6 space-y-2 group">
                <h3 class="h4 group-hover:underline">{link.name}</h3>
                <p class="opacity-70">{link.description}</p>
            </a>
        {/each}
